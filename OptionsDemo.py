...
class OptionsMarket(dag.DomainObj):
    ...
    @dag.DagMethod
    def vol(self, strike = 1, tenor = 1, strike_type = 'pct'):
    ...
    def PV(self, SpotOverRide = None, VolOverRide = None, VolOverRideType = 'shock'):
        ...
        if VolOverRide != None:
            if VolOverRideType.upper() == 'SHOCK':
                strike_vol = self.options_market().vol(self.strike,tenor = (self.expiry - self.value_date() ), strike_type = 'abs')+VolOverRide
            else:
                strike_vol = VolOverRide

        else:
            strike_vol = self.options_market().vol(self.strike,tenor = (self.expiry - self.value_date() ), strike_type = 'abs')
...
option1.value_date.set_value(ql.Date())
